...
| **Philippines DPA** | § 21 keeps the PIC accountable for data transferred to a PIP — there is no whitelist and no SCC regime, so the basis is "contractual or other reasonable means" plus documented due diligence. § 14 requires the outsourcing agreement to be in writing and to bind the PIP to the PIC's security obligations. For private-sector data sharing see NPC Circular 2020-03. The vendor must be able to support **72-hour** breach notice to NPC *and* subjects (NPC Circular 16-03). |
