...
2. **Malaysia PDPA, Philippines DPA, and Vietnam PDPD jurisdiction content.** Tagged for v0.2 — see [CHANGELOG.md](CHANGELOG.md). Each needs the same shape as the existing `jurisdictions/sg-pdpa/`, `th-pdpa/`, and `id-pdp/` folders: a `README.md` with statute version metadata, an `obligations/` folder mapping the statute to the universal layers, and a `statute-map.md` reverse lookup.
...
- **Non-SEA jurisdictions.** This repo is scoped to SEA personal-data-protection statutes: SG PDPA, TH PDPA, ID UU PDP today, with MY PDPA, PH DPA, and VN PDPD planned for v0.2. GDPR, UK GDPR, CCPA, etc. are intentionally excluded — they are significantly different statutes and would dilute the skill. A separate project per jurisdiction family is a better pattern.
