...
1. **Statute updates.** When PDPC (Singapore), the appointed Indonesia DPA, PDPC (Thailand), JPDP (Malaysia), or NPC (Philippines) publishes an amendment or material new guidance, open a PR updating the relevant jurisdiction file plus the [CHANGELOG](CHANGELOG.md) entry. Cite the official source and link to the regulator's announcement.
2. **Vietnam PDPL jurisdiction content.** Tagged for v0.5 — see [CHANGELOG.md](CHANGELOG.md). Written against **Law 91/2025/QH15** (in force 1 January 2026) and **Decree 356/2025/ND-CP**, not the superseded Decree 13/2023/ND-CP. Needs the same shape as the existing `skills/personal-data-protection/jurisdictions/sg-pdpa/`, `th-pdpa/`, `id-pdp/`, `my-pdpa/`, and `ph-dpa/` folders: a `README.md` with statute version metadata, an `obligations/` folder mapping the statute to the universal layers, and a `statute-map.md` reverse lookup.
...
- **Non-SEA jurisdictions.** This repo is scoped to SEA personal-data-protection statutes: SG PDPA, TH PDPA, ID UU PDP, MY PDPA, and PH DPA today, with VN PDPL planned for v0.5. GDPR, UK GDPR, CCPA, etc. are intentionally excluded — they are significantly different statutes and would dilute the skill. A separate project per jurisdiction family is a better pattern.
